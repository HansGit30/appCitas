    ...
    <div class="h-[calc(100vh-140px)] flex flex-col justify-center items-center px-4">
        ...
        <div class="w-full max-w-2xl bg-white border border-slate-100 rounded-[32px] p-8 shadow-xl shadow-slate-100/70 relative">
            ...
            <div class="mb-8 text-center">
                ...
                <h2 class="text-xl font-extrabold text-slate-800 tracking-tight">Editar Doctor</h2>
                <p class="text-xs text-slate-400 mt-1">Actualice la información del médico en el sistema.</p>
            </div>

            {{-- 1. Ruta dirigida al método update y método PUT --}}
            <form action="{{ route('medico.update', $doctor->id_medico) }}" method="POST" class="space-y-5">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-x-5 gap-y-4">
                    ...
                    <div class="flex flex-col gap-1.5">
                        ...
                        <input type="tel" id="telefono" name="telefono" value="{{ old('telefono', $doctor->telefono) }}"
                               class="w-full bg-slate-50/60 border border-slate-200 rounded-xl px-4 py-2.5 text-sm text-slate-800 focus:outline-none focus:border-blue-500 transition-all">
                    </div>
                    ...
                    <div class="flex flex-col gap-1.5">
                        ...
                        <input type="text" id="licencia" name="licencia" value="{{ old('licencia', $doctor->licencia) }}" required
                               class="w-full bg-slate-50/60 border border-slate-200 rounded-xl px-4 py-2.5 text-sm text-slate-800 focus:outline-none focus:border-blue-500 transition-all">
                    </div>

                    <div class="flex flex-col gap-1.5 md:col-span-2">
                        <label for="años_experiencia" class="text-xs font-bold text-slate-700 ml-1">Años de Experiencia</label>
                        <input type="number" id="años_experiencia" name="años_experiencia" value="{{ old('años_experiencia', $doctor->{'años_experiencia'}) }}"
                               class="w-full bg-slate-50/60 border border-slate-200 rounded-xl px-4 py-2.5 text-sm text-slate-800 focus:outline-none focus:border-blue-500 transition-all">
                    </div>
